...
def make_features(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # One-hot encode categoricals after combining train and forecast rows so feature columns align.
...
def main(
    ...
    raw = build_raw_df(engine, seasons=seasons_to_load, gw_max=last_gw)
    ...
    # Forecast next GW using target-season feature rows at last_gw.
    target_gw = last_gw + 1
    forecast = feat[
    ...
    if forecast.empty:
        raise RuntimeError(
            f"No feature rows for season={resolved_target_season}, last_gw={last_gw}. "
            "This script requires target-season actual rows through last_gw. "
            "For a true pre-GW1 forecast, use the future pre-GW1 prediction mode instead."
        )

    yhat = model.predict(forecast[feature_cols].to_numpy())

    out = pd.DataFrame(
        {
            "season": resolved_target_season,
            "player_id": forecast["player_id"].astype(int),
    ...
    print(
        ...
        f"wrote {len(out)} rows for target_gw={target_gw} "
        f"model={MODEL_NAME} forecast_from_last_gw={last_gw}"
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=int,
        required=True,
        help="Latest GW with actuals in player_gw_stats for the target season.",
    )
    ap.add_argument(
        "--split_gw",
        "--split-gw",
