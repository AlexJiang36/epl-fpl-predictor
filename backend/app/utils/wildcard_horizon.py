...
def load_horizon_prediction_rows(
    *,
    ...
    stmt = (
        ...
        .where(
            Prediction.season == resolved_season,
...
def load_recent_minutes_by_player(
    *,
    ...
    stmt = (
        ...
        .where(
            PlayerGameweekStat.season == resolved_season,
...
def aggregate_player_horizon_features(
    ...
    for pred, pl, tm in rows:
        row_season = season or getattr(pred, "season", None) or get_current_season()
...
def build_wildcard_horizon_snapshot(
    *,
    ...
    return {
        ...
        "assumptions": {
            "future_vs_historical_planning": "Uses currently stored future prediction rows from the selected model for the requested season and GW horizon.",
            "minutes_stability_source": "Computed from historical player_gw_stats in the same season before start_gw.",
